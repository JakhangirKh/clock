...
function clock() {
    ...
    //inner.HTML - это св-во, которое позволяет получить или изменить HTML - содержимое эл-та 
...
tabsItem.forEach((item, i) => {
    item.addEventListener('click', () => {
        addRemoveActive(item, tabsItem)
        addRemoveActive(tabsContentItem[i], tabsContentItem)
    })
})

// classList - это св-во, которое позволяет получить доступ к классам эл-та

// add - добавляет класс
// remove - удаляет класс

function addRemoveActive(element, arr) {
    arr.forEach(item => {
